...
**Исключение — страницы с загрузкой файла (аудиозаписи).** Web3Forms на
бесплатном тарифе файлы не принимает, поэтому такие страницы шлют файлы через
Basin, прошитый прямо в их собственном `<script>`, не через `test-engine.js`.
Сейчас на один и тот же адрес — `https://usebasin.com/f/6e8f507d4a7d` —
завязаны: `ege-2027/Демо/2027-demo-oral` (устный демо-вариант ЕГЭ, запись
ответов на 4 задания) и все 5 материалов `oxford-phonics/3/...` (Unit 2
Long i + 4 воркшита Unit 3 — по одной записи «прочитай вслух» на воркшит,
поле `reading_aloud`). Курсы различаются в письме по полю `course`/`exercise`,
а не по адресу — заводить новый Basin-адрес под новый курс не обязательно,
можно продолжать вешать на тот же (30 августа 2026 — добавлены Oxford Phonics
World 3).
